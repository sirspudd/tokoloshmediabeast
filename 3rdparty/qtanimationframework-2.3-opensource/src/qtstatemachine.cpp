                                           ...
                                           const QList<QtAbstractState*> &enteredStates)
{
    ...
    for (int i = 0; i < resetEndValues.size(); ++i)
        qobject_cast<QtVariantAnimation*>(resetEndValues.at(i))->setEndValue(QVariant()); // ### generalize
    resetEndValues.clear();

    // Find the animations to use for the state change.
    QList<QtAbstractAnimation*> selectedAnimations;
    ...
    for (int i = 0; i < selectedAnimations.size(); ++i) {
        QtAbstractAnimation *anim = selectedAnimations.at(i);
        QList<QPropertyAssignment>::iterator it;
        for (it = propertyAssignments.begin(); it != propertyAssignments.end(); ) {
            QPair<QList<QtAbstractAnimation*>, QList<QtAbstractAnimation*> > ret;
            ret = initializeAnimation(anim, *it);
            QList<QtAbstractAnimation*> handlers = ret.first;
            if (!handlers.isEmpty()) {
        ...
        // ### generalize
        QList<QtVariantAnimation*> variantAnims = qFindChildren<QtVariantAnimation*>(anim);
        if (QtVariantAnimation *va = qobject_cast<QtVariantAnimation*>(anim))
...
QPair<QList<QtAbstractAnimation*>, QList<QtAbstractAnimation*> >
QtStateMachinePrivate::initializeAnimation(QtAbstractAnimation *abstractAnimation, 
                                          const QPropertyAssignment &prop)
{
    QList<QtAbstractAnimation*> handledAnimations;
    QList<QtAbstractAnimation*> localResetEndValues;
...
void QtStateMachinePrivate::_q_animationFinished()
{
    ...
    QList<QPair<QtAbstractAnimation*, QPropertyAssignment> >::iterator it;
    for (it = propertiesForAnimations.begin(); it != propertiesForAnimations.end(); ) {
        QtAbstractAnimation *a = (*it).first;
